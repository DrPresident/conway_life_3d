...
func tick(x, y, z):
    ...
    elif grid[x][y][cur] == ALIVE and (life == 2 or life == 3):
        grid[x][y][nxt] = ALIVE

    elif grid[x][y][cur] == DEAD and life == 3:
        grid[x][y][nxt] = ALIVE

    else:
        grid[x][y][nxt] = grid[x][y][cur]

    if grid[x][y][nxt] == ALIVE:
        var new_unit = unit.instance()
        #new_unit.transform.translated(Vector3(x, y, z))
        new_unit.translation = Vector3(x, z, y)
        add_child(new_unit)
        #new_unit.show()
        print('alive! ', new_unit.translation)
